...
function getInfo (url,data,method,header) {
        ...
        wx.request({
            ...
            success:function(res){
                ...
                if (res.statusCode == 200) {
                    ...
                    }else if (res.data.api_message == 402) {
                        wx.showModal({
                            title: '',
                            content: '先去绑定手机号吧！',
                            success: function (res) {
  ...
  /**
 * 封封微信的的request async异步函数
 */

async function asyncRequest(url, data = {}, method = "POST", header = "application/x-www-form-urlencoded"){
    try{
        let response =await getInfo(url,data,method , header);
        return response
    }catch(e){
        console.log(e);
        return e
